...
private:
	...
	//regex match
	std::wsmatch priv_match;

public:
	//user defined variables
	int pub_speed_mode = 1;
	std::basic_regex<wchar_t> pub_search_filename_regex;
	std::string pub_mode;
	...
	void set_variables(const int speed_mode_input, const std::string mode_input, const std::wstring dir_to_search_input, void(*output_function_input)(std::vector <std::wstring> &output), std::basic_regex<wchar_t> * search_filename_regex_input) {
		pub_speed_mode = speed_mode_input;
		pub_mode = mode_input;
		pub_search_filename_regex = *search_filename_regex_input;
		...
		if (!home_dir.empty()) {
			...
			if (find_handle == INVALID_HANDLE_VALUE) {}
				...
				if (file_exists(result_full)) {
					...
					if (pub_mode == "search") {
						try {
							const std::basic_regex<wchar_t> filename_regex(L"*.txt");
							if (std::regex_search(result_only, priv_match, filename_regex) && priv_match.size() > 1) {
								LOG_SPAM(result_only + L" matches the regex");
							...
							LOG_ERROR("SOMETHING WENT WRONG WITH YOUR REGEX " + error);
						}
					}
					else {
						EnterCriticalSection(&priv_files_sync);
